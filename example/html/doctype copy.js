...
window.onload = function () {
    let dogo = window.localStorage.getItem('dogName');
    let clickSerch = window.localStorage.getItem('clickEvent');
    let clickImage = window.localStorage.getItem('clickPicture');
    if(clickSerch != null) {
        for (i = 0; i < dpTemp.length; i++) {
            if (dogo == dpTemp[i][0]) {
                $(".explainName").text(dpTemp[i][0]);
        ...
        window.localStorage.removeItem('dogName');//처: https://hianna.tistory.com/697 [어제 오늘 내일:티스토리]
        window.localStorage.removeItem('clickSerch');
    }
